...
那么下面我们就得重点讲解下抽象语法树（下面表述中就直接用它的简称 AST 了），看看什么是 [AST](00-前端/00-核心/JavaScript/核心概念/00-收集箱/碎片（临时内容）/抽象语法树：Abstract%20Syntax%20Tree,AST.md) 以及 AST 的生成过程是怎样的。
...
其实字节码配合解释器和编译器是最近一段时间很火的技术，比如 Java 和 Python 的虚拟机也都是基于这种技术实现的，我们把这种技术称为[即时编译](00-前端/00-核心/JavaScript/核心概念/编译原理/即时编译JIT.md)（JIT）。具体到 V8，就是指解释器 Ignition 在解释执行字节码的同时，收集代码信息，当它发现某一部分代码变热了之后，TurboFan 编译器便闪亮登场，把热点的字节码转换为机器码，并把转换后的机器码保存起来，以备下次使用。
...
![](附件/87f55e00-ca25-4857-9724-78d7efac2d68_1739515856613.png)
