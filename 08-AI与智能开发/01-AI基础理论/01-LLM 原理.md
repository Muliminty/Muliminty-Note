...
输入: "Hello, 你好世界"
Token: ["Hello", ",", " 你好", "世界"]
ID:    [15496, 11, 57668, 20126]
```

关键概念：

- **Token ≠ 字 ≠ 词**：一个 Token 可能是一个字、半个词、一个标点，甚至一个字节
- **Subword 分词**：主流方案（BPE、SentencePiece）通过统计频率自动学习分词粒度
...
- `temperature=0`：贪心解码，每次选概率最高的 Token → 输出确定、保守
- `temperature=0.7`：适度随机 → 平衡创造性与准确性
- `temperature=1.5`：高度随机 → 更有创造力但可能偏离主题

### 4. 自回归生成

LLM 的生成是**逐 Token 进行**的：

```
Prompt: "人工智能的核心是"
→ 预测 Token 1: "机"
→ 将"机"拼接到上文，预测 Token 2: "器"
→ 将"器"拼接到上文，预测 Token 3: "学"
→ 将"学"拼接到上文，预测 Token 4: "习"
→ ...直到生成终止符 <EOS> 或达到 Max Tokens
```

这就是为什么 LLM 的回复是"一个字一个字蹦出来"的——它本质上就是在一个一个地预测。
