...
举个例子： **未优化代码：依赖数组配置错误**，如下代码，`useEffect` 在每次组件渲染时都会执行。当 `name` 更新时，尽管与 `count` 无关，副作用仍会被触发。怎么配置不用多说了吧。
...
##### **5.1 使用 `React.lazy` 和 `Suspense` 进行懒加载**
