...
> 当前决策边界（2026-04-18）：
> 本轮治理明确不修改 `.obsidian/app.json` 与 `quartz.config.ts`。
> 因此，“更新笔记新建路径”与“发布边界防泄漏配置”两项暂保留为未完成状态，但属于已知且暂缓处理的配置决策，不视为执行遗漏。
...
### 0.2 知识图谱信息模型升级
- [x] **增加生命周期颗粒度 (Status 机制)**：已在 [01-百科写作规范.md](./01-百科写作规范.md) 中加入 `status` 字段及其受控状态值（草稿 / 成熟 / 常青 / 归档）。
- [x] **建立黑白名单标签字典**：已在 [01-百科写作规范.md](./01-百科写作规范.md) 中补充标签受控规则、推荐白名单与常见黑名单示例。
...
- [x] **优先补齐 `03-前端开发/03-工程化` 的 Frontmatter**：`README.md`、核心 `!MOC` 与当前主干正文已完成 Frontmatter 补齐，后续遗留问题转入死链与内容深修阶段。
- [x] **优先补齐 `03-前端开发/04-质量保障` 的 Frontmatter**：`README.md` 与四个子专题 `!MOC` 已完成 Frontmatter 补齐，后续遗留问题转入坏链与正文深化阶段。
- [x] **优先收口 `04-质量保障` 的缺失文件与入口坏链**：已补齐测试、性能、安全、无障碍首批主题文件，并完成目录入口层路径修正，当前入口坏链已清零。
- [x] **继续收口 `03-工程化` 的缺失文件与路径失效**：已完成 `Webpack / ESBuild / Gulp / Vite / Rollup / Babel / 包管理` 入口层与高频正文的路径纠偏，当前扫描仅剩代码块正则示例被误判的伪坏链。
- [x] **继续收口 `02-前端框架` 的缺失文件与路径失效**：已完成 `README`、`React / Vue / Angular / Next.js / 状态管理 / Tailwind CSS` 入口层纠偏，补齐 `Vue`、`Angular`、`Tailwind CSS`、`状态管理` 首批缺失主题文件，并完成 `React Native` 子树 29 篇缺失正文落地；当前结构坏链已清零。
- [x] **继续收口 `06-架构与工程/07-团队协作` 的缺失文件与路径失效**：已补齐 `团队协作 / 运营 / 合规` 三个子专题的首批正式文件，当前结构坏链已清零。
- [x] **继续收口 `03-前端开发/05-高级应用` 的缺失文件与路径失效**：已完成 `01-架构`、`02-网络与传输`、`03-部署与发布`、`04-动画与交互`、`05-移动与跨平台`、`06-前沿` 的入口纠偏与首批缺失正文补齐，并修正 `README.md` 旧路径；当前结构坏链已清零。
- [x] **继续收口 `01-计算机基础` 的缺失文件与路径失效**：已完成 `01-计算机组成原理`、`02-数据结构与算法`、`03-操作系统`、`04-计算机网络`、`05-编译原理` 的首批缺失正文补齐与入口纠偏，并补建 `!MOC-Linux.md`、`!MOC-WSL.md` 以承接子树回链；当前 `01-计算机基础` 的双链与核心 MOC 结构校验已通过。
- [x] **继续收口 `07-基础设施与运维` 的缺失文件与路径失效**：已修正 Docker 归档中的旧双链、Nginx 子路径编码问题，并补齐 `!MOC-基础设施与运维.md`、`!MOC-Nginx.md`、`!MOC-Docker.md` 的现有文件挂载；当前 `07-基础设施与运维` 的双链与核心 MOC 结构校验已通过。
- [x] **继续收口 `08-AI与智能开发` 的缺失文件与路径失效**：已补建 `!MOC-OpenClaw.md` 以承接工具生态子树回链，并补齐 `!MOC-AI与智能开发.md` 的完整索引挂载；当前 `08-AI与智能开发` 的双链与核心 MOC 结构校验已通过。
- [x] **继续收口 `09-工具与效率` 的缺失文件与路径失效**：已修正顶层入口中的旧代码管理路径，并补齐 `!MOC-工具与效率.md` 与 `!MOC-代码管理.md` 的现有文件挂载；当前 `09-工具与效率` 的双链与核心 MOC 结构校验已通过。
- [x] **清理旧入口与显性死链残留**：已移除 `00-总览与索引/!MOC-总览与索引.md`、`10-实战案例/!MOC-实战案例.md` 中失效的旧入口，并修正 `10-实战案例/01-前端实战/前端大文件上传.md`、`09-工具与效率/01-开发工具/Cursor 使用技巧.md`、`07-基础设施与运维/03-容器化/Docker/01-基础入门/02-核心概念.md` 等正文中的旧目录死链；同时补建 `10-实战案例/01-前端实战/!MOC-前端实战.md`，当前 `10-实战案例` 的双链与核心 MOC 结构校验已通过。
- [x] **完成全库双链扫描与真伪分流**：本轮起始扫描结果为 `118` 条，其中 `70` 条属于真实缺链，`48` 条属于模板占位符、示例写法或语法误判；完成本轮修复后，当前扫描仅剩 `48` 条伪报错。
- [x] **先修 `JavaScript` 和 `React` 那批真实缺链**：已修正 `JavaScript` 主树与遗留归档中的事件循环、对象、Cookie 相关失效链接，补齐 `React` 基础入门中的 `Props / State / 条件渲染 / 列表渲染 / 组件组合 / 事件处理 / Key` 基础节点，并补建 `11-问题排错/05-代码流排错` 的 `README.md` 与 `!MOC-代码流排错.md`；当前这批真实缺链已清零。
- [x] **为系统规范引入 `status` 字段**：已在 [01-百科写作规范.md](./01-百科写作规范.md) 中补充生命周期状态字段，如“草稿 / 成熟 / 常青 / 归档”。
- [x] **建立受控标签字典**：已将官方标签白名单或受控词表写入系统规范，抑制 `front-end`、`前端`、`FE` 等并行标签扩散。
- [x] **执行一次全库死链扫描**：已完成一次全库级扫描，并据此生成“真实问题 / 伪报错”分流结果，后续转入分批修复阶段。
- [x] **回收 TODO 中过时的旧路径欠账**：已清理自动扫描结果中已不再真实存在的历史路径，并将仍然有效的问题重写为当前路径。

### 0.4 当前剩余事项优先级（2026-04-28）

1. **大面积死链清洗**：当前真实缺链已基本清零，下一步应单独处理模板占位符、示例语法与扫描误判规则，避免继续把伪报错计入缺陷。
2. **确认资源图片存储策略并回写规范文档**：当前不改 `.obsidian/app.json`，但要尽快把“分散存放 / 未来收敛”的制度口径写死。
3. **强化“原子节点-知识主干”中间件连接**：补上 `![[原子概念]]` 的使用边界和主干文章嵌入策略。
4. **清点全库所有非标准目录**：收掉残余特殊目录和过渡目录，避免结构继续漂移。
5. **全库统一中英文混排空格规范**：在大结构稳定后推进排版一致性，避免大面积返工。
6. **全库统一同层目录的排序风格**
7. **全库统一同层文件的排序风格**
8. **Frontmatter 存量补齐**：按专题分批推进，不再和结构修复混做。
...
- [x] 按 [03-百科迁移方案.md](./03-百科迁移方案.md) 分批迁移旧一级目录
- [ ] 清点全库所有非标准目录
...
> 本节保留历史迁移记录，用于复盘“旧专题如何并入新骨架”。
> 若条目中的旧目录已废止，默认将其视为历史来源路径，而非当前执行路径。
...
- [x] 新建 `05-问题排查`
- [x] 将根目录平铺文件迁入新分层
- [x] 更新原 `!MOC-经验技巧.md` 以匹配新结构
- [x] 更新原 `README.md` 以匹配新结构
- [x] 规范部分文件名中的技术名大小写
- [x] 规范部分文件名中的中英文空格
...
> 本节用于记录旧专题的迁移执行过程。
> 标题中的旧一级目录名称保留为“迁移来源标识”，当前实际整理应以新目录为准。
...
- [x] 按照《迁移方案》，将 `03-工程化实践` 全部迁入 `03-前端开发/03-工程化`
...
- [x] 检查是否需要补目录说明文件
- [x] 按照《迁移方案》，将本目录迁入 `06-架构与工程/07-团队协作`
- [x] 清理迁移后残留的 `00-前端体系` 空目录及旧入口文件，彻底废止该一级目录
...
- [x] 检查当前文件命名是否符合新规范
- [x] 将 `Git`、`GitHub` 及日常协作工具主题迁入 `09-工具与效率/02-代码管理`
- [x] 将 分支策略、规范制度 等内容迁入 `06-架构与工程/03-工程规范`
- [x] 删除空目录与遗留入口文件后废止本一级目录
...
- [x] 检查 `08-监控与日志`
- [x] 检查 `09-架构设计`
- [x] 将 `09-架构设计` 整体迁入 `06-架构与工程/01-软件架构`
- [x] 检查并迁移其他服务端残留分支（如安全、优化模块需重新分配）
- [x] 余项清点完毕且目录均被搬空后，删除入口文件并彻底废止 `03-服务端` 一级目录
...
- [x] 彻底删除 `04-AI与机器学习` 旧根目录残留（如已过时的 `!MOC` 及根入口文件）
...
- [x] 清理旧目录中的空目录
- [x] 彻底删除 `05-AI辅助开发` 旧根目录残留（如旧的 `!MOC` 与根入口文件）
...
> 2026-04-21 已回收 `03-前端开发/03-工程化` 目录中已补齐 Frontmatter 的历史扫描项。
> 2026-04-21 已回收 `03-前端开发/04-质量保障` 目录与 `03-工程化` 尾部残留的已修复 Frontmatter 扫描项。
...
- [ ] [规范问题] 文件缺失 Frontmatter: `09-工具与效率/02-代码管理/01-GitHub开源贡献指南.md`
- [ ] [规范问题] 文件缺失 Frontmatter: `09-工具与效率/02-代码管理/!MOC-代码管理.md`
