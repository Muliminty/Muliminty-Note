...
The data flows down. The server is the source of truth. Components that want to "receive props from the server" opt into that with the [`'use client'` directive](../what-does-use-client-do/index.md). Since our "Server Loaders" are indistinguishable from components, we're calling them Server Components. But they're a composable version of Server Loaders.
