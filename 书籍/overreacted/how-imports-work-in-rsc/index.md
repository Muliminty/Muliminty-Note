...
I've [previously written](../what-does-use-client-do/index.md) about the `'use client'` and `'use server'` directives which mark the "split points" between the two environments. In this post, I'd like to focus on *how* these directives interact with the `import` and `export` keywords.
...
It would be nice if there was some way for the backend code to *refer* to `sayHello` without bringing it *into* the backend. Luckily, that is what [`'use client'` does](../what-does-use-client-do/index.md):
