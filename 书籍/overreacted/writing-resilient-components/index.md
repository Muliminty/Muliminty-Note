...
However, one *could* design an API that *can* be statically analyzed for consistency. The [React `useEffect` Hook](../a-complete-guide-to-useeffect/index.md) is an example of such API:
...
![Demo of exhaustive-deps lint rule](./img/useeffect.gif)
...
I don’t suggest a dogmatic interpretation of these rules. Of course, in a simpler app you might want to use local state for everything, including those “caches”. I’m only talking about the ideal user experience [from the first principles](../the-elements-of-ui-engineering/index.md).
...
**These principles help you write components that are [optimized for change](../optimized-for-change/index.md). It’s easy to add, change them, and delete them.**
