...
tags: ["Docker", "核心概念", "基础入门", "容器化"]
moc: "[[!MOC-Docker]]"
stage: "基础入门"
prerequisites: []
description: "说明 Docker 中镜像、容器、仓库等基础概念及其关系，建立容器化的最小认知模型。"
publish: true
aliases: ["核心概念", "Docker 概念基础"]
toc: true
---
...
2. **容器修改不影响镜像**
   - 容器中的修改只存在于可写层
   - 删除容器后，修改会丢失
   - 如果需要保存数据，需要使用[数据卷](../03-容器管理/02-容器数据管理.md)
...
**步骤**：
1. 运行交互式容器：`docker run -it --name my-ubuntu ubuntu:20.04 /bin/bash` [20260130-运行交互式容器](../../99-参考与归档/01-批注/20260130-运行交互式容器.md)
2. 在容器内执行命令：
   - `ls` - 查看文件列表
   - `pwd` - 查看当前目录
   - `whoami` - 查看当前用户
...
- [03-基本命令.md](./03-基本命令.md) - 学习更多 Docker 命令
- [../03-容器管理/01-容器基础操作.md](../03-容器管理/01-容器基础操作.md) - 学习如何管理容器
- [../02-镜像管理/03-镜像基础操作.md](../02-镜像管理/03-镜像基础操作.md) - 学习如何管理镜像
- [../02-镜像管理/05-Dockerfile编写指南.md](../02-镜像管理/05-Dockerfile编写指南.md) - 学习如何创建自己的镜像
