...
不过，在介绍[进程和线程](../../../00-编程/前端提高-计算机原理/进程和线程.md)之前，我需要先讲解下什么是并行处理，因为如果你理解了并行处理的概念，那么再理解进程和线程之间的关系就会变得轻松许多。

### 什么是并行处理

计算机中的[并行处理](../../../00-编程/前端提高-计算机原理/什么是并行处理.md)就是同一时刻处理多个任务，比如我们要计算下面这三个表达式的值，并显示出结果
